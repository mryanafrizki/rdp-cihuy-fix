...
export interface PaymentData {
  ...
  created_at: string;
  expired_at: string;
  // Pakasir-specific fields
  _pakasir_project?: string;
  _pakasir_order_id?: string;
  _pakasir_amount?: number;
}

export interface PaymentResult {
  success: boolean;
  data?: PaymentData;
  error?: string | { code: number; status: number; message: string };
  errorResponse?: any;
}
...
export type PaymentGateway = 'atlantich2h' | 'pakasir';
...
interface RateLimiter {
  ...
  maxPerSecond: number;
  requests: number[];
  cleanOldRequests(): void;
  canMakeRequest(): boolean;
  addRequest(): void;
  waitForSlot(maxWait?: number): Promise<boolean>;
}
...
// ============================================================================
// Atlantic H2H Payment Functions
// ============================================================================

// Create payment via Atlantic
async function createPaymentAtlantic(apiKey: string, reffId: string, amount: number): Promise<PaymentResult> {
  try {
...
async function checkPaymentStatusAtlantic(apiKey: string, transactionId: string, retryCount = 0): Promise<PaymentStatusResult> {
  try {
...
async function createPaymentPakasir(apiKey: string, projectSlug: string, orderId: string, amount: number): Promise<PaymentResult> {
  try {
...
async function checkPaymentStatusPakasir(apiKey: string, projectSlug: string, orderId: string, amount: number, retryCount = 0): Promise<PaymentStatusResult> {
  try {
    // PENTING: Rate limiting untuk prevent API overload
...
export async function createPayment(apiKey: string, reffId: string, amount: number, additionalData: CreatePaymentOptions = {}): Promise<PaymentResult> {
...
export async function checkPaymentStatus(apiKey: string, transactionId: string, retryCount = 0, additionalData: CheckStatusOptions = {}): Promise<PaymentStatusResult> {
  const gateway = getPaymentGateway();
...
export function isPaymentStatusSuccessful(status: string | PaymentStatusResult | { status: string } | { data: { status: string } }): boolean {
  // Terima berbagai format input
  let statusString = '';
  
  if (typeof status === 'string') {
    statusString = status.toLowerCase();
  } else if (status && typeof status === 'object' && 'status' in status && typeof status.status === 'string') {
    statusString = status.status.toLowerCase();
  } else if (status && typeof status === 'object' && 'data' in status && status.data && typeof status.data === 'object' && 'status' in status.data && typeof status.data.status === 'string') {
