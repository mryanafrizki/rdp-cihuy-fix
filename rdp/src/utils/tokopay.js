...
class TokoPay {
  ...
  async createPayment(amount, method = 'QRIS') {
    console.info('🚀 Creating payment request:', { amount });
    ...
    try {
      ...
      console.info('✅ Payment creation successful:', response.data);
      ...
      this.monitorPaymentStatus({
        refId,
        onSuccess: (status) => console.info('💰 Payment completed:', { refId, status }),
        onTimeout: () => console.info('⏰ Payment monitoring timeout:', { refId }),
  ...
  async checkPaymentStatus(refId) {
    console.info('🔍 Checking payment status:', { refId });
    ...
    try {
      const response = await axios.get(`${this.baseUrl}/order`, { params });
      console.info('✅ Status check result:', { refId, status: response.data.status });
  ...
  async monitorPaymentStatus({
    ...
    console.info('🔄 Starting payment monitoring:', { refId });
    let attempts = 0;
