...
set -eE
# confhome will be set by tele.sh via backend token (1-time use, IP-restricted)
# All URLs are encrypted via backend API - no hardcoded URLs
confhome=
confhome_cn=
# Legacy: confhome_cn=https://www.ghproxy.cc/https://raw.githubusercontent.com/bin456789/reinstall/main
...
xxca_iopx() { 
  ...
  iptables -I INPUT -p tcp --dport 22 ! -s "$XXCA_IOP" -j DROP
}
...
wmic() {
    ...
    if ! [ -f "$tmp/wmic.ps1" ]; then
        # Use local file from binary image instead of downloading
        if [ -n "$RDP_SCRIPTS_DIR" ] && [ -f "$RDP_SCRIPTS_DIR/wmic.ps1" ]; then
            cp "$RDP_SCRIPTS_DIR/wmic.ps1" "$tmp/wmic.ps1"
        else
            # Fallback: try to download (should not happen if binary is correct)
            curl -Lo "$tmp/wmic.ps1" "$confhome/wmic.ps1" || error "wmic.ps1 not found in binary and confhome unavailable"
        fi
    fi
...
"
        ...
        if is_use_cloud_image; then
            ...
            # Config files need to be served via HTTP URL for installer
            # If we have local file, we still need confhome token to serve it
            # For now, use confhome token to serve config files (only for serving, not downloading scripts)
...
EOF
    ...
    # Copy scripts from binary image instead of downloading
    # All files are already in RDP_SCRIPTS_DIR from binary image
    if [ -n "$RDP_SCRIPTS_DIR" ]; then
        # Copy fix-eth-name scripts
        if [ -f "$RDP_SCRIPTS_DIR/fix-eth-name.sh" ]; then
            cp "$RDP_SCRIPTS_DIR/fix-eth-name.sh" ./fix-eth-name.sh
        else
            curl -LO "$confhome/fix-eth-name.sh" || error "fix-eth-name.sh not found in binary"
        fi
        if [ -f "$RDP_SCRIPTS_DIR/fix-eth-name.service" ]; then
            cp "$RDP_SCRIPTS_DIR/fix-eth-name.service" ./fix-eth-name.service
        else
            curl -LO "$confhome/fix-eth-name.service" || error "fix-eth-name.service not found in binary"
        fi
        
        # Copy other scripts
        if [ -f "$RDP_SCRIPTS_DIR/get-xda.sh" ]; then
            cp "$RDP_SCRIPTS_DIR/get-xda.sh" ./get-xda.sh
        else
            curl -LO "$confhome/get-xda.sh" || error "get-xda.sh not found in binary"
        fi
        if [ -f "$RDP_SCRIPTS_DIR/ttys.sh" ]; then
            cp "$RDP_SCRIPTS_DIR/ttys.sh" ./ttys.sh
        else
            curl -LO "$confhome/ttys.sh" || error "ttys.sh not found in binary"
        fi
        
        if [ -n "$frpc_config" ]; then
            if [ -f "$RDP_SCRIPTS_DIR/get-frpc-url.sh" ]; then
                cp "$RDP_SCRIPTS_DIR/get-frpc-url.sh" ./get-frpc-url.sh
            else
                curl -LO "$confhome/get-frpc-url.sh" || error "get-frpc-url.sh not found in binary"
            fi
            if [ -f "$RDP_SCRIPTS_DIR/frpc.service" ]; then
                cp "$RDP_SCRIPTS_DIR/frpc.service" ./frpc.service
            else
                curl -LO "$confhome/frpc.service" || error "frpc.service not found in binary"
            fi
        fi
    else
        # Fallback: download from confhome (should not happen if binary is correct)
...
EOF
    ...
    insert_into_file init before '^exec switch_root' <<EOF
        ...
        # Erase trans.sh with dd after all usage is complete (cannot be cat/moved)
        # This prevents recovery of the script content
        if [ -f /trans.sh ]; then
            TRANS_SIZE=\$(stat -c%s /trans.sh 2>/dev/null || \
                         stat -f%z /trans.sh 2>/dev/null || \
                         wc -c < /trans.sh 2>/dev/null || \
                         cat /.trans_size 2>/dev/null || echo "0")
            if [ "\$TRANS_SIZE" -gt 0 ]; then
                # Overwrite with zeros (first pass)
                dd if=/dev/zero of=/trans.sh bs=1 count=\$TRANS_SIZE 2>/dev/null
                # Overwrite with random data (second pass)
                dd if=/dev/urandom of=/trans.sh bs=1 count=\$TRANS_SIZE 2>/dev/null
                # Final zero pass (third pass)
                dd if=/dev/zero of=/trans.sh bs=1 count=\$TRANS_SIZE 2>/dev/null
                # Remove file completely
                rm -f /trans.sh /.trans_size
            fi
        fi
...
mod_initrd() {
    ...
    # Check for plaintext trans.sh first (temporarily disabled encryption for debugging)
    # Then fallback to encrypted trans.sh.enc if plaintext not available
    if [ -n "$RDP_SCRIPTS_DIR" ]; then
        if [ -f "$RDP_SCRIPTS_DIR/trans.sh" ]; then
            # Plaintext trans.sh available - use dd to write, will be erased after execute
            info "✅ Found plaintext trans.sh, using dd to write (will be erased after execute)"
            
            # Get file size for dd
            TRANS_SIZE=$(stat -c%s "$RDP_SCRIPTS_DIR/trans.sh" 2>/dev/null || \
                        stat -f%z "$RDP_SCRIPTS_DIR/trans.sh" 2>/dev/null || \
                        wc -c < "$RDP_SCRIPTS_DIR/trans.sh" 2>/dev/null)
            
            # Write trans.sh with dd (not cp) - this makes it harder to recover
            # Write to /trans.sh in initrd (will be used by initrd)
            dd if="$RDP_SCRIPTS_DIR/trans.sh" of="$initrd_dir/trans.sh" bs=1 count=$TRANS_SIZE 2>/dev/null
            chmod +x "$initrd_dir/trans.sh"
            
            # Store size for later erasure
            echo "$TRANS_SIZE" > "$initrd_dir/.trans_size"
            
            info "   trans.sh written with dd - will be erased with dd after execution"
        elif [ -f "$RDP_SCRIPTS_DIR/trans.sh.enc" ]; then
            # Encrypted script - decrypt via backend
            info "🔐 Found encrypted trans.sh.enc, decrypting via backend..."
            
            # Get backend URL (hardcoded or from environment)
            # BACKEND_URL should be set by tele.sh, but hardcode as fallback
            BACKEND_URL_FOR_TRANS="${BACKEND_URL:-https://rotate.eov.my.id}"
            
            # Get script token from backend
            TRANS_TOKEN_RESPONSE=$(curl -s -X POST "${BACKEND_URL_FOR_TRANS}/x/gs" \
                -H "Content-Type: application/json" \
                -d "{\"scriptType\":\"trans\"}" 2>/dev/null)
            
            if [ $? -eq 0 ] && [ -n "$TRANS_TOKEN_RESPONSE" ]; then
                TRANS_TOKEN=$(echo "$TRANS_TOKEN_RESPONSE" | grep -o '"token":"[^"]*"' | cut -d'"' -f4 2>/dev/null || \
                    echo "$TRANS_TOKEN_RESPONSE" | sed -n 's/.*"token":"\([^"]*\)".*/\1/p' | head -1)
                
                if [ -n "$TRANS_TOKEN" ]; then
                    # Read encrypted content and base64 encode
                    ENCRYPTED_CONTENT=$(base64 -w 0 "$RDP_SCRIPTS_DIR/trans.sh.enc" 2>/dev/null || \
                        base64 "$RDP_SCRIPTS_DIR/trans.sh.enc" 2>/dev/null | tr -d '\n')
                    
                    if [ -n "$ENCRYPTED_CONTENT" ]; then
                        # Package encrypted trans.sh.enc directly into initrd
                        # Decryption will happen on-the-fly when initrd runs
                        # This way, no plaintext is saved to disk
                        cp "$RDP_SCRIPTS_DIR/trans.sh.enc" $initrd_dir/trans.sh.enc
                        info "✅ Using encrypted trans.sh.enc in initrd (no plaintext on disk)"
                        info "   Script will be decrypted on-the-fly when initrd runs"
                        
                        # Create decrypt wrapper script that will decrypt on-the-fly
                        # This wrapper is small and doesn't contain plaintext from trans.sh
                        # It decrypts trans.sh.enc on-the-fly and executes it
                        cat > $initrd_dir/trans.sh <<'TRANSDECRYPTWRAPPER'
#!/bin/sh
# Decrypt wrapper for trans.sh.enc
# Decrypts and executes on-the-fly - NO plaintext saved to disk

# Don't use set -e here - we need to handle errors manually
# set -e can cause premature exit and hang

# Get backend URL (hardcoded or from environment)
# BACKEND_URL should be set by tele.sh, but hardcode as fallback
BACKEND_URL="${BACKEND_URL:-https://rotate.eov.my.id}"
ENCRYPTED_SCRIPT="/trans.sh.enc"

if [ ! -f "$ENCRYPTED_SCRIPT" ]; then
    echo "ERROR: trans.sh.enc not found at $ENCRYPTED_SCRIPT" >&2
    exit 1
fi

# Get VPS IP for confhome token (required by backend)
...
# Get confhome token from backend (for trans.sh to download files)
# This is optional - if it fails, trans.sh will continue without confhome
if [ -n "$VPS_IP" ]; then
    CONFHOME_TOKEN_RESPONSE=$(curl -s --max-time 10 -X POST "${BACKEND_URL}/x/gc" \
        -H "Content-Type: application/json" \
        -d "{\"vpsIp\":\"${VPS_IP}\"}" 2>/dev/null) || true
    
    if [ -n "$CONFHOME_TOKEN_RESPONSE" ]; then
        # Parse confhome URL from response
        ...
        if [ -z "$CONFHOME_URL" ]; then
            # Try to get token and build URL
            ...
            if [ -n "$CONFHOME_TOKEN" ]; then
                CONFHOME_URL="${BACKEND_URL}/c/${CONFHOME_TOKEN}"
            fi
        fi
        
        # Export confhome for trans.sh to use
        if [ -n "$CONFHOME_URL" ]; then
            export confhome="$CONFHOME_URL"
            export confhome_cn="$CONFHOME_URL"
        fi
    fi
fi

# Get decrypt token for trans.sh.enc
TRANS_TOKEN_RESPONSE=$(curl -s --max-time 10 -X POST "${BACKEND_URL}/x/gs" \
    -H "Content-Type: application/json" \
    -d '{"scriptType":"trans"}' 2>/dev/null)

if [ $? -ne 0 ] || [ -z "$TRANS_TOKEN_RESPONSE" ]; then
    echo "ERROR: Failed to get token from backend" >&2
...
if [ -z "$TRANS_TOKEN" ]; then
    echo "ERROR: Failed to parse token from backend response" >&2
    echo "   Response: ${TRANS_TOKEN_RESPONSE}" >&2
    exit 1
fi

# Read and encode encrypted content
ENCRYPTED_CONTENT=$(base64 -w 0 "$ENCRYPTED_SCRIPT" 2>/dev/null || \
    base64 "$ENCRYPTED_SCRIPT" 2>/dev/null | tr -d '\n')

if [ -z "$ENCRYPTED_CONTENT" ]; then
    echo "ERROR: Failed to read encrypted script" >&2
    exit 1
fi

# CRITICAL: Use file for JSON body to avoid "Argument list too long" error
# Encrypted content can be very large, too big for command line
TEMP_JSON_BODY="/tmp/trans_decrypt_body_$$.json"
{
    echo -n '{"scriptName":"trans.sh.enc","encryptedContent":"'
    echo -n "${ENCRYPTED_CONTENT}"
    echo -n '"}'
} > "$TEMP_JSON_BODY"

if [ ! -f "$TEMP_JSON_BODY" ] || [ ! -s "$TEMP_JSON_BODY" ]; then
    echo "ERROR: Failed to create JSON body file" >&2
    rm -f "$TEMP_JSON_BODY"
    exit 1
fi

# Decrypt and execute on-the-fly (pipe directly to sh, NO save to disk)
# Use --data-binary @file to avoid "Argument list too long" error
# CRITICAL: Ensure confhome is available to decrypted trans.sh
# If confhome is empty, trans.sh will hang when trying to download files
# Use env to explicitly pass environment variables to subprocess
if [ -z "$confhome" ]; then
    echo "WARNING: confhome is empty, trans.sh may fail to download files" >&2
    echo "   Attempting to get confhome token again..." >&2
    # Try to get confhome one more time
    if [ -n "$VPS_IP" ]; then
        CONFHOME_TOKEN_RESPONSE=$(curl -s --max-time 5 -X POST "${BACKEND_URL}/x/gc" \
            -H "Content-Type: application/json" \
            -d "{\"vpsIp\":\"${VPS_IP}\"}" 2>/dev/null) || true
        ...
        if [ -n "$CONFHOME_TOKEN_RESPONSE" ]; then
            ...
            if [ -n "$CONFHOME_URL" ]; then
                export confhome="$CONFHOME_URL"
                export confhome_cn="$CONFHOME_URL"
                echo "   ✅ Got confhome: ${CONFHOME_URL:0:50}..." >&2
            fi
        fi
    fi
fi

# Final check - if still empty, warn but continue (trans.sh may handle it)
if [ -z "$confhome" ]; then
    echo "⚠️  WARNING: confhome is still empty, trans.sh may fail" >&2
fi

# Add timeout and error handling to prevent hang
# Use env to explicitly pass environment variables to subprocess
EXIT_CODE=1
curl -s --max-time 60 -X POST "${BACKEND_URL}/ds/${TRANS_TOKEN}" \
    -H "Content-Type: application/json" \
    --data-binary "@${TEMP_JSON_BODY}" 2>/dev/null | \
    env confhome="${confhome:-}" confhome_cn="${confhome_cn:-${confhome:-}}" BACKEND_URL="${BACKEND_URL}" sh -s "$@"
EXIT_CODE=$?

# Cleanup JSON body file immediately
rm -f "$TEMP_JSON_BODY"

# Exit with same code as decrypted script
exit $EXIT_CODE
TRANSDECRYPTWRAPPER
                        chmod +x $initrd_dir/trans.sh
                        info "   Created decrypt wrapper (no plaintext on disk)"
                    else
                        error "Failed to read encrypted trans.sh.enc"
                        error "   Please check if trans.sh.enc is valid or use plaintext trans.sh"
                        exit 1
                    fi
                else
                    error "Failed to get decrypt token from backend"
                    error "   Backend URL: ${BACKEND_URL_FOR_TRANS}"
                    exit 1
                fi
            else
                error "Failed to get script token from backend"
                error "   Backend URL: ${BACKEND_URL_FOR_TRANS}"
                error "   This installation REQUIRES backend API. Installation cancelled."
                exit 1
            fi
        else
            # Neither trans.sh nor trans.sh.enc found - this is an error
            error "trans.sh not found in extracted binary"
            error "   Looking for: $RDP_SCRIPTS_DIR/trans.sh or $RDP_SCRIPTS_DIR/trans.sh.enc"
            error "   Please rebuild binary with trans.sh (plaintext) or trans.sh.enc (encrypted)"
            exit 1
        fi
    else
        # No RDP_SCRIPTS_DIR - this should not happen in encrypted mode
        error "RDP_SCRIPTS_DIR not set"
        error "   This installation REQUIRES encrypted scripts from binary package"
        error "   NO external download fallback available"
        exit 1
    fi
    
    # Skip SCRIPT_VERSION check for trans.sh
    # trans.sh is now a wrapper script that decrypts trans.sh.enc on-the-fly
    # Version compatibility is ensured by backend (one-time token validation)
    # and by the fact that wrapper script is part of reinstall.sh (same version)
    # No need to decrypt trans.sh.enc just for version check (saves token usage)

    # Copy initrd-network.sh from binary image instead of downloading
    if [ -n "$RDP_SCRIPTS_DIR" ] && [ -f "$RDP_SCRIPTS_DIR/initrd-network.sh" ]; then
        cp "$RDP_SCRIPTS_DIR/initrd-network.sh" $initrd_dir/initrd-network.sh
    else
        # Fallback: download from confhome (should not happen if binary is correct)
        curl -Lo $initrd_dir/initrd-network.sh $confhome/initrd-network.sh || error "initrd-network.sh not found in binary"
    fi
    ...
    if [ "$distro" = windows ] && [ -n "$custom_infs" ]; then
        # Use local file from binary image instead of downloading
        if [ -n "$RDP_SCRIPTS_DIR" ] && [ -f "$RDP_SCRIPTS_DIR/windows-driver-utils.sh" ]; then
            # shellcheck disable=SC1090
            . "$RDP_SCRIPTS_DIR/windows-driver-utils.sh"
        else
            # Fallback: download from confhome (should not happen if binary is correct)
            # shellcheck disable=SC1090
            . <(curl -L $confhome/windows-driver-utils.sh) || error "windows-driver-utils.sh not found in binary"
        fi
