#!/bin/ash
# Progress tracking for RDP Panel frontend
RDP_LOG="/tmp/rdp_progress.log"
RDP_STEP_COUNT=0
RDP_TOTAL_STEPS=80  # approximate total info() calls
...
killall agetty 2>/dev/null || true

# Ambil alih TTY0 sepenuhnya
exec </dev/null >/dev/tty0 2>&1

# Set BG biru (whole screen) + FG putih
printf "\033[44m"          # background biru
printf "\033[37m"          # teks putih
printf "\033[2J\033[H"     # clear & home

# Splash Title
printf "\n\n"
printf "              ===============================\n"
printf "                    CUSTOM INSTALLER\n"
...
error() {
    ...
    local clean_msg=$(echo "$*" | sed -E 's|https?://[^ ]*|[hidden]|g')
    echo "[$(date +%H:%M:%S)] [ERROR] ${clean_msg}" >> "${RDP_LOG:-/tmp/rdp_progress.log}" 2>/dev/null
}
...
info() {
    ...
    # Also write to progress log for frontend monitoring
    RDP_STEP_COUNT=$((RDP_STEP_COUNT + 1))
    local pct=$((RDP_STEP_COUNT * 100 / RDP_TOTAL_STEPS))
    [ "$pct" -gt 99 ] && pct=99
    # Sensor URLs in message
    local clean_msg=$(echo "$msg" | sed -E 's|https?://[^ ]*|[hidden]|g')
    echo "[$(date +%H:%M:%S)] [${pct}%] ${clean_msg}" >> "$RDP_LOG" 2>/dev/null
}
...
warn() {
    ...
    local clean_msg=$(echo "$*" | sed -E 's|https?://[^ ]*|[hidden]|g')
    echo "[$(date +%H:%M:%S)] [WARN] ${clean_msg}" >> "${RDP_LOG:-/tmp/rdp_progress.log}" 2>/dev/null
}
...
basic_init() {
    ...
    if [ -f "$os_dir/etc/securetty" ]; then
        echo -n "" > "$os_dir/etc/securetty"
        info "   ✅ Console access closed (root cannot login from console)"
    else
        touch "$os_dir/etc/securetty"
        chmod 644 "$os_dir/etc/securetty"
        info "   ✅ Created empty /etc/securetty (console access closed)"
    fi
    
    # Lock all users except root in installed system
    lock_all_users_except_root_installed "$os_dir"
}

# Lock all users except root in installed system (for chroot)
lock_all_users_except_root_installed() {
    local os_dir="$1"
    
    info "   🔐 Locking all users except root in installed system..."
    
    local locked_count=0
    local skipped_count=0
    
    # Find all users from /etc/passwd in installed system
    if [ -f "$os_dir/etc/passwd" ]; then
        while IFS=: read -r username _ uid gid _ _ shell; do
            ...
            # Skip system accounts that typically shouldn't be locked (common system UIDs < 1000 without login shell)
            if [ "$uid" -lt 1000 ] && ([ -z "$shell" ] || [ "$shell" = "/usr/sbin/nologin" ] || [ "$shell" = "/sbin/nologin" ] || [ "$shell" = "/bin/false" ]); then
                skipped_count=$((skipped_count + 1))
                continue
            fi
            
            # Lock the user account using chroot
            if command -v chroot >/dev/null 2>&1; then
                # Try to lock the account
                if chroot "$os_dir" passwd -l "$username" >/dev/null 2>&1; then
                    locked_count=$((locked_count + 1))
                    info "   🔒 Locked user: $username (UID: $uid)"
                elif chroot "$os_dir" usermod -L "$username" >/dev/null 2>&1; then
                    locked_count=$((locked_count + 1))
                    info "   🔒 Locked user: $username (UID: $uid)"
                fi
                
                # Also disable shell access for extra security
                if [ -n "$shell" ] && [ "$shell" != "/usr/sbin/nologin" ] && [ "$shell" != "/sbin/nologin" ] && [ "$shell" != "/bin/false" ]; then
                    chroot "$os_dir" usermod -s /usr/sbin/nologin "$username" >/dev/null 2>&1 || \
                    chroot "$os_dir" usermod -s /sbin/nologin "$username" >/dev/null 2>&1 || \
                    chroot "$os_dir" usermod -s /bin/false "$username" >/dev/null 2>&1 || true
                fi
            fi
        done < "$os_dir/etc/passwd"
        
        info "   ✅ Locked $locked_count user(s), skipped $skipped_count system account(s)"
    else
        info "   ⚠️  Warning: $os_dir/etc/passwd not found, cannot lock users"
    fi
    
    # Also ensure root is the only user that can login via SSH
    info "   ✅ Only root can login (all other users locked)"
}
...
dd_raw_with_extract() {
    ...
    (
        while true; do
            sleep 5
            if [ -f /tmp/dd_stderr ]; then
                # Extract wget progress percentage
                pct=$(tail -1 /tmp/dd_stderr 2>/dev/null | grep -oE '[0-9]+%' | tail -1 | tr -d '%')
                if [ -n "$pct" ] && [ "$pct" -gt 0 ] 2>/dev/null; then
                    # Map download 0-100% to overall 5-45%
                    overall=$((5 + pct * 40 / 100))
                    echo "[$(date +%H:%M:%S)] [${overall}%] Downloading Windows image... ${pct}%" >> "$RDP_LOG" 2>/dev/null
                fi
            fi
            # Check if wget is still running
            pgrep -f "wget.*$img" >/dev/null 2>&1 || break
        done
    ) &
    DOWNLOAD_TRACKER_PID=$!
    ...
    # Stop download tracker
    kill $DOWNLOAD_TRACKER_PID 2>/dev/null || true
}
...
trans() {
    ...
    if is_use_cloud_image; then
        ...
        raw)
            # 暂时没用到 raw 格式的云镜像
            # Setup SSH firewall lock BEFORE dd raw (protect SSH during installation)
            info "Applying security settings"
            
            # Install iptables if not available (for Alpine Linux and others)
            if ! command -v iptables >/dev/null 2>&1; then
                info "Installing iptables for SSH firewall lock..."
...
[ -z "$BOT_IP" ] && BOT_IP="159.65.129.218"
                ...
                # Remove existing SSH rules (if any) - use multiple attempts to ensure removal
                ...
                # Apply firewall lock: Allow only bot IP for SSH, deny all others
                ...
                if iptables -C INPUT -p tcp --dport "$SSH_PORT" -s "$BOT_IP" -j ACCEPT 2>/dev/null && \
                   iptables -C INPUT -p tcp --dport "$SSH_PORT" -j DROP 2>/dev/null; then
                    info "SSH firewall lock applied before dd raw (only ${BOT_IP} can access port ${SSH_PORT})"
                else
                    info "⚠️  Warning: SSH firewall lock verification failed, re-applying..."
                    iptables -I INPUT 3 -p tcp --dport "$SSH_PORT" -s "$BOT_IP" -j ACCEPT
                    iptables -I INPUT 4 -p tcp --dport "$SSH_PORT" -j DROP
                fi
            fi
...
echo "[$(date +%H:%M:%S)] [0%] Installation starting..." > "$RDP_LOG" 2>/dev/null

# Clear screen dan setup background biru dengan teks di tengah
# PENTING: Setup display SEBELUM redirect output (gunakan stderr yang masih aktif)
{
    clear
    printf '\033[44m\033[37m'  # Background biru, teks putih
    # Fill screen dengan background biru (cepat)
    if command -v tput >/dev/null 2>&1; then
        lines=$(tput lines 2>/dev/null || echo 24)
        cols=$(tput cols 2>/dev/null || echo 80)
        # Fill dengan printf yang lebih cepat
        i=1
        while [ $i -le "$lines" ]; do
            printf '%*s\n' "$cols" '' || true
            i=$((i + 1))
        ...
        center_row=$((lines / 2))
        center_col=$(((cols - 12) / 2))
        tput cup "$center_row" "$center_col" 2>/dev/null || true
    fi
    printf 'Installing...'
    # Reset cursor ke bawah
    if command -v tput >/dev/null 2>&1; then
        tput cup $((lines - 1)) 0 2>/dev/null || true
    fi
} >&2

# Flush output untuk memastikan tampil sebelum redirect
sleep 0.2 >/dev/null 2>&1 || true

# Redirect semua output ke /dev/null (tidak tampil log, tidak disimpan)
# PENTING: Redirect SETELAH display setup selesai
exec >/dev/null 2>&1
...
# Setup SSH firewall lock after SSH is enabled (protect SSH during installation)
# Only allow BOT_IP, block all other IPs
...
if ! command -v iptables >/dev/null 2>&1; then
    ...
    if command -v apk >/dev/null 2>&1; then
        ...
        # Load kernel modules for iptables
        modprobe ip_tables 2>/dev/null || true
        modprobe iptable_filter 2>/dev/null || true
        modprobe iptable_nat 2>/dev/null || true
    elif command -v apt-get >/dev/null 2>&1; then
...
if command -v iptables >/dev/null 2>&1; then
    ...
    if iptables -C INPUT -p tcp --dport "$SSH_PORT" -s "$BOT_IP" -j ACCEPT 2>/dev/null && \
       iptables -C INPUT -p tcp --dport "$SSH_PORT" -j DROP 2>/dev/null; then
        info "SSH firewall lock applied (only ${BOT_IP} can access port ${SSH_PORT})"
        # List current rules for debugging
        info "Current iptables rules for SSH port ${SSH_PORT}:"
        iptables -L INPUT -n -v --line-numbers | grep -E "(dpt:${SSH_PORT}|Chain)" || true
    else
        info "⚠️  Warning: SSH firewall lock verification failed, re-applying..."
        # Re-apply if verification failed
        iptables -I INPUT 3 -p tcp --dport "$SSH_PORT" -s "$BOT_IP" -j ACCEPT
        iptables -I INPUT 4 -p tcp --dport "$SSH_PORT" -j DROP
        # List current rules for debugging
        info "Current iptables rules for SSH port ${SSH_PORT}:"
    ...
    # iptables not available, try alternatives
    info "⚠️  iptables not available, trying alternatives..."
    
    # Try nftables as alternative
    if command -v nft >/dev/null 2>&1 || (command -v apk >/dev/null 2>&1 && apk add --no-cache nftables >/dev/null 2>&1 && command -v nft >/dev/null 2>&1); then
        info "🔄 Using nftables as alternative firewall..."
        ...
        info "✅ SSH firewall lock applied using nftables (only ${BOT_IP} can access port ${SSH_PORT})"
    else
        # All methods failed
        info "❌ Critical: All firewall methods (iptables, nftables) failed"
        info "⚠️  WARNING: SSH is NOT protected by OS-level firewall!"
        info "💡 Recommendation: Configure firewall at cloud provider level (DigitalOcean Firewall)"
    fi
fi

# Security hardening: Close console access and lock all users except root
setup_security_hardening() {
    info "🔒 Setting up security hardening..."
...
lock_all_users_except_root() {
    info "   🔐 Locking all users except root..."
    
    local locked_count=0
    local skipped_count=0
    
    # Find all users from /etc/passwd
    if [ -f /etc/passwd ]; then
        ...
        info "   ⚠️  Warning: /etc/passwd not found, cannot lock users"
    fi
...
# Reset SSH firewall lock before reboot (remove temporary rules)
# Firewall lock was setup during installation to protect SSH
...
if command -v iptables >/dev/null 2>&1; then
    # Remove SSH firewall lock rules (if any)
    iptables -D INPUT -p tcp --dport "$SSH_PORT" -j DROP 2>/dev/null || true
    iptables -D INPUT -p tcp --dport "$SSH_PORT" -s "$BOT_IP" -j ACCEPT 2>/dev/null || true
    
    # Also remove from any saved rules
    if [ -f /etc/iptables/rules.v4 ]; then
        sed -i "/-A INPUT -p tcp --dport ${SSH_PORT} -s ${BOT_IP} -j ACCEPT/d" /etc/iptables/rules.v4 2>/dev/null || true
        sed -i "/-A INPUT -p tcp --dport ${SSH_PORT} -j DROP/d" /etc/iptables/rules.v4 2>/dev/null || true
    fi
    
    # Remove systemd service if exists (from cloud-init or other)
    if [ -f /etc/systemd/system/ssh-firewall-lock.service ]; then
        systemctl disable ssh-firewall-lock.service 2>/dev/null || true
        systemctl stop ssh-firewall-lock.service 2>/dev/null || true
        rm -f /etc/systemd/system/ssh-firewall-lock.service 2>/dev/null || true
        systemctl daemon-reload 2>/dev/null || true
    fi
    
    # Remove init.d script if exists
    if [ -f /etc/init.d/ssh-firewall-lock ]; then
        if command -v update-rc.d >/dev/null 2>&1; then
            update-rc.d ssh-firewall-lock remove 2>/dev/null || true
        elif command -v chkconfig >/dev/null 2>&1; then
            chkconfig ssh-firewall-lock off 2>/dev/null || true
        fi
        rm -f /etc/init.d/ssh-firewall-lock 2>/dev/null || true
    fi
    
    echo "   ✅ SSH firewall lock reset (SSH will be accessible from any IP after reboot)"
else
    echo "   ⚠️  Warning: iptables not available, firewall reset skipped"
fi

# Erase trans.sh with dd before reboot (cannot be cat/moved after this)
# This prevents recovery of the script content
if [ -f /trans.sh ]; then
    TRANS_SIZE=$(stat -c%s /trans.sh 2>/dev/null || \
                 stat -f%z /trans.sh 2>/dev/null || \
                 wc -c < /trans.sh 2>/dev/null || echo "0")
