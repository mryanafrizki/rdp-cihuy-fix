...
class DigitalOceanClient {
  ...
  async request(method, path, body = null) {
    return new Promise((resolve, reject) => {
        ...
        if (this.proxy.protocol === 'socks4' || this.proxy.protocol === 'socks5') {
          if (SocksProxyAgent && typeof SocksProxyAgent === 'function') {
            try {
              ...
              console.log(`[DigitalOcean API] Using SOCKS proxy: ${this.proxy.protocol}://${this.proxy.host}:${this.proxy.port}`);
            } catch (error) {
          ...
          if (HttpsProxyAgent && typeof HttpsProxyAgent === 'function') {
            try {
              ...
              console.log(`[DigitalOcean API] Using HTTPS proxy: ${this.proxy.host}:${this.proxy.port}`);
            } catch (error) {
          ...
          if (HttpProxyAgent && typeof HttpProxyAgent === 'function') {
            try {
              ...
              console.log(`[DigitalOcean API] Using HTTP proxy: ${this.proxy.host}:${this.proxy.port}`);
            } catch (error) {
      ...
      console.log(`[DigitalOcean API] ${method} ${fullUrl}${this.proxy ? ' (via proxy)' : ''}`);
        ...
        res.on('end', () => {
          try {
            ...
            if (res.statusCode >= 200 && res.statusCode < 300) {
              console.log(`[DigitalOcean API] ✅ ${method} ${fullUrl} - ${res.statusCode}`);
              resolve({ status: res.statusCode, body: jsonData });
            } else {
              console.log(`[DigitalOcean API] ❌ ${method} ${fullUrl} - ${res.statusCode}: ${jsonData.message || 'Error'}`);
              console.log(`[DigitalOcean API] Full error response:`, JSON.stringify(jsonData, null, 2));
            ...
            console.log(`[DigitalOcean API] ❌ Parse error: ${error.message}`);
      ...
      req.on('error', (error) => {
        console.log(`[DigitalOcean API] ❌ Network error: ${error.message}`);
        reject(error);
      });

      if (body) {
        const bodyString = JSON.stringify(body);
        console.log(`[DigitalOcean API] Request body:`, bodyString);
  ...
  async getBalance() {
    ...
    console.log('[DigitalOcean API] Balance response:', JSON.stringify(response.body, null, 2));
  ...
  async listSizes() {
    ...
    console.log(`[DigitalOcean API] Fetched ${allSizes.length} total sizes across ${page} pages`);
    return allSizes;
  }
