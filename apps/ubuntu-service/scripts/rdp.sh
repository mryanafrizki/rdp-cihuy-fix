...
get_automated_input() {
    ...
    case $WINDOWS_ID in
        ...
        12) VERSION="2022"; VERSION_NAME="Windows Server 2022";;
        13) VERSION="2019"; VERSION_NAME="Windows Server 2019";;
        14) VERSION="2016"; VERSION_NAME="Windows Server 2016";;
        15) VERSION="2012"; VERSION_NAME="Windows Server 2012";;
        16) VERSION="2008"; VERSION_NAME="Windows Server 2008";;
        17) VERSION="2025"; VERSION_NAME="Windows Server 2025";;
        18) VERSION="2003"; VERSION_NAME="Windows Server 2003";;
        19) VERSION="tiny11"; VERSION_NAME="Tiny11 (Lightweight Win11)";;
    ...
    # Disk: if 0 or empty, auto-detect available disk (leave 10GB for OS)
    if [[ "$STORAGE_INPUT" == "0" || -z "$STORAGE_INPUT" ]]; then
        AUTO_DISK=$(df -BG / | awk 'NR==2{print $4}' | sed 's/G//')
        DISK_SIZE="$((AUTO_DISK > 10 ? AUTO_DISK - 10 : AUTO_DISK))G"
        echo "Auto-detected disk: ${DISK_SIZE}" >&2
    else
...
# Fungsi untuk memindahkan SSH ke port 2222 agar port 22 bisa dipakai RDP
migrate_ssh_port() {
    echo -e "${YELLOW}Migrating SSH to port 2222...${NC}"
    echo "Migrating SSH to port 2222..." >&2

    # Backup sshd_config
    cp /etc/ssh/sshd_config /etc/ssh/sshd_config.bak

    # Cek apakah sudah di port 2222
    if grep -q "^Port 2222" /etc/ssh/sshd_config; then
        echo "SSH already on port 2222" >&2
        return 0
    fi

    # Ubah port SSH ke 2222
    ...
    if sshd -t 2>/dev/null; then
        ...
        # Wait for port 22 to fully release
        echo "Waiting for port 22 to release..." >&2
        for i in $(seq 1 15); do
            if ! ss -tlnp 2>/dev/null | grep -q ':22 '; then
                echo "Port 22 released after ${i}s" >&2
                break
            fi
            sleep 1
        done
        # Final check
        if ss -tlnp 2>/dev/null | grep -q ':22 '; then
            echo "WARNING: Port 22 still occupied after 15s, forcing..." >&2
            fuser -k 22/tcp 2>/dev/null || true
            sleep 2
        fi
        echo -e "${GREEN}SSH migrated to port 2222${NC}"
        echo "SSH migrated to port 2222" >&2
    else
        echo "sshd config test failed, reverting..." >&2
        cp /etc/ssh/sshd_config.bak /etc/ssh/sshd_config
        echo -e "${RED}SSH migration failed, keeping port 22${NC}"
        echo "SSH migration failed, keeping port 22" >&2
        return 1
    fi
}
...
EOF
    ...
    # OEM install script: rename PC, change RDP port, disable lockout, extend disk, etc.
    cat > oem/install.bat << 'OEMEOF'
@echo off
mode con cp select=437 >nul

rem ========= CONFIGURATION =========
rem RDP port stays 3389 INSIDE container. Docker maps host:22 -> container:3389.
rem Do NOT change RdpPort here — it breaks the Docker port mapping.
set "RdpPort=3389"
set "NEWNAME=COBAIN-DEV-DOCKER"
...
rem ========= CHANGE RDP PORT =========
echo [i] Setting RDP port to %RdpPort% ...
...
rem ========= EXTEND DISK =========
echo [i] Extending disk partition...
...
rem ========= FRESH RDP CERTIFICATE =========
echo [i] Removing RDP certificate for fresh fingerprint...
reg delete "HKLM\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp" /v SSLCertificateSHA1Hash /f >nul 2>&1
del /f /q "%ProgramData%\Microsoft\Crypto\RSA\MachineKeys\*" >nul 2>&1

rem ========= REBOOT =========
shutdown /r /t 5 /c "Applying RDP + Rename Settings"
del "%~f0"
OEMEOF
...
show_access_info() {
    ...
    echo -e "${CYAN}RDP: ${SERVER_IP}:22${NC}"
    echo -e "${CYAN}Username: ${USERNAME}${NC}"
...
main() {
    ...
    # Migrate SSH to port 2222 to free port 22 for RDP
    if ! migrate_ssh_port; then
        echo "WARNING: SSH migration failed, port 22 may still be occupied" >&2
        # Try to kill whatever is on port 22 as last resort
        fuser -k 22/tcp 2>/dev/null || true
        sleep 3
    fi
    
    # Verify port 22 is free before proceeding
    if ss -tlnp 2>/dev/null | grep -q ':22 '; then
        echo "ERROR: Port 22 still in use! Cannot bind Docker RDP." >&2
        ss -tlnp 2>/dev/null | grep ':22 ' >&2
        exit 1
    fi
    echo "Port 22 is free, proceeding..." >&2
